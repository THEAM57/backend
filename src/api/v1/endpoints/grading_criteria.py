...
@router.get("/{criteria_id}", response_model=GradingCriteriaResponse, summary="Получить критерий по ID")
...
@router.patch("/{criteria_id}", response_model=GradingCriteriaResponse, summary="Обновить критерий")
...
@router.delete("/{criteria_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Удалить критерий")
...
@router.get("/project-type/{project_type_id}/total-score", summary="Получить максимальную сумму баллов")
async def get_total_max_score(
    ...
    return {"project_type_id": project_type_id, "total_max_score": total}
