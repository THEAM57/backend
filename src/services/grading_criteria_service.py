...
class GradingCriteriaService:
    ...
    async def create_criteria(self, criteria_data: GradingCriteriaCreate) -> GradingCriteria:
        ...
        exists = await self.repository.exists_by_name(criteria_data.project_type_id, criteria_data.name)
        if exists:
    ...
    async def get_by_project_type(self, project_type_id: int) -> GradingCriteriaListResponse:
    ...
    async def update_criteria(self, criteria_id: int, criteria_data: GradingCriteriaUpdate) -> GradingCriteria:
        """Обновить критерий
